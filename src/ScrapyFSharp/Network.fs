...
module Network =
        ...
        member private x.ExecuteRequest (url:Uri, verb:HttpVerb, data:HttpRequestData option, iteration) =
            async {
                let path = 
                    ...
                    | _ -> url
                let request = x.CreateRequest path verb
                let! body =
                    async {
                        match verb with
                        | Put
    ...
    type BrowserConfig = 
        { UserAgent:FakeUserAgent
          Proxy:IWebProxy
          DecompressionMethod:DecompressionMethods option
          Language:CultureInfo
          CachePolicy:RequestCachePolicy option
          Timeout:TimeSpan
          KeepAlive:bool
          ...
          AllowMetaRedirect:bool
          SendChunked:bool
          IgnoreCookies:bool
          UseDefaultCookiesParser:bool }
        static member Default = 
            { UserAgent = FakeUserAgent.Chrome24
              Proxy = WebRequest.DefaultWebProxy
              DecompressionMethod = Some DecompressionMethods.GZip
    ...
    let browser f =
        ...
        let c = f BrowserConfig.Default
        b.UserAgent <- c.UserAgent
        b.Proxy <- c.Proxy
        b.DecompressionMethod <- c.DecompressionMethod
        b.Language <- c.Language
        b.CachePolicy <- c.CachePolicy
        b.Timeout <- c.Timeout
        b.KeepAlive <- c.KeepAlive
        b.ProtocolVersion <- c.ProtocolVersion
        b.AutoDetectCharsetEncoding <- c.AutoDetectCharsetEncoding
        b.Encoding <- c.Encoding
        b.AllowAutoRedirect <- c.AllowAutoRedirect
        b.AllowMetaRedirect <- c.AllowMetaRedirect
        b.SendChunked <- c.SendChunked
        b.IgnoreCookies <- c.IgnoreCookies
        b.UseDefaultCookiesParser <- c.UseDefaultCookiesParser
        b
